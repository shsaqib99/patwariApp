                        ...
                        <div class="card-body">
                            <div class="table-responsive p-0">
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tbody>
                                    @foreach($khaivet as $key => $item)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->mauza->name }}</td>
                                            <td>{{ $item->mauza->patwar_circle->name }}</td>
                                            <td>{{ $item->mauza->patwar_circle->Qanoongoi->name }}</td>
                                            <td>{{ $item->mauza->patwar_circle->Qanoongoi->Tehsil->name }}</td>
                                            <td>{{ $item->mauza->patwar_circle->Qanoongoi->Tehsil->District->name }}</td>
                                            <td>
                                                <div class="btn-group-sm">
                                                    <a href="{{ route('dashboard.khaivet.edit',$item->id) }}" class="btn btn-success">
                                                        <i class="fa fa-edit"></i>
                                                    </a>

                                                    <button type="button" onclick="deleteFun('{{ $item->id }}','{{ route('dashboard.khaivet.destroy',$item->id) }}')" class="btn btn-danger">
    ...
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('dashboard.khaivet.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        ...
                        <input type="text" id="name" name="name" placeholder="Khaivet" class="form-control">
                    </div>
