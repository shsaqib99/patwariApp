                        ...
                        <div class="card-body">
                            <div class="table-responsive p-0">
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tbody>
                                    @foreach($khatooni as $key => $item)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->khaivet->name }}</td>
                                            <td>{{ $item->khaivet->mauza->name }}</td>
                                            <td>{{ $item->khaivet->mauza->patwar_circle->name }}</td>
                                            <td>{{ $item->khaivet->mauza->patwar_circle->Qanoongoi->name }}</td>
                                            <td>{{ $item->khaivet->mauza->patwar_circle->Qanoongoi->Tehsil->name }}</td>
                                            <td>{{ $item->khaivet->mauza->patwar_circle->Qanoongoi->Tehsil->District->name }}</td>
                                            <td>
                                                <div class="btn-group-sm">
                                                    <a href="{{ route('dashboard.khatooni.edit',$item->id) }}" class="btn btn-success">
                                                        <i class="fa fa-edit"></i>
                                                    </a>

                                                    <button type="button" onclick="deleteFun('{{ $item->id }}','{{ route('dashboard.khatooni.destroy',$item->id) }}')" class="btn btn-danger">
    ...
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('dashboard.khatooni.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        ...
                        <label for="name">Khatooni</label>
                        <input type="text" id="name" name="name" placeholder="Khatooni" class="form-control">
                    </div>
