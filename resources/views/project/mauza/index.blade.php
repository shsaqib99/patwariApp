                        ...
                        <div class="card-body">
                            <div class="table-responsive p-0">
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tbody>
                                    @foreach($mauza as $key => $item)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->patwar_circle->name }}</td>
                                            <td>{{ $item->patwar_circle->Qanoongoi->name }}</td>
                                            <td>{{ $item->patwar_circle->Qanoongoi->Tehsil->name }}</td>
                                            <td>{{ $item->patwar_circle->Qanoongoi->Tehsil->District->name }}</td>
                                            <td>
                                                <div class="btn-group-sm">
                                                    <a href="{{ route('dashboard.mauza.edit',$item->id) }}" class="btn btn-success">
                                                        <i class="fa fa-edit"></i>
                                                    </a>

                                                    <button type="button" onclick="deleteFun('{{ $item->id }}','{{ route('dashboard.mauza.destroy',$item->id) }}')" class="btn btn-danger">
    ...
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('dashboard.mauza.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        ...
                        <input type="text" id="name" name="name" placeholder="Mauza Name" class="form-control">
                    </div>
